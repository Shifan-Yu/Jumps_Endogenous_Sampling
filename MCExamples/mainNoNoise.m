...
[x1, xc, xd1, v, N1, ~] = simPriceEfficient(num,rep,initial,mu,kappa,theta,xi,rho,lambda,beta1);
[~,  ~,  xd2, ~, N2, ~] = simPriceEfficient(num,rep,initial,mu,kappa,theta,xi,rho,lambda,beta2);
x2 = xc + xd2;                          % larger-jump alternative

%% ------------------------------------------------------------------------
%  Barrier family: c = K * sqrt(Var(Δ_i^n X))
% -------------------------------------------------------------------------
